...
async function updateProgram(req, res) {
  try {
        ...
        mailer.sendMail(
          {
            from: process.env.MAIL_SENDER,
            to: x.email,
            subject: `There is Some Update Regarding Our Program : Team ${process.env.SITE_NAME}`,
            text: `
              Hello Guardians,
              Please There is Some Update Regarding Our Latest Program
    ...
    error.errors?.name ? (errorMessage.name = error.errors.name.message) : null;
    error.errors?.seats
      ? (errorMessage.seats = error.errors.seats.message)
      : null;
    error.errors?.cover
      ? (errorMessage.cover = error.errors.cover.message)
      : null;
