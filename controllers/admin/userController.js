  ...
  if (req.body.password === req.body.cpassword) {
    if (schema.validate(req.body.password)) {
      ...
      res.render("admin/user/create", {
        ...
        errorMessage: {
          password:
            "Invalid Password. It Must contains at least 1 uppercase and 1 lowercase letter, 1 digit, should not contain any space and length must be 8-100",
        },
        data: data,
        session: req.session,
      });
...
async function forgetPassword2Store(req, res) {
  try {
    ...
    if (data) {
      ...
      res.render("admin/user/forget-password2", {
        title: "Admin - Reset Password",
        errorMessage: "UnAuthorized Activity",
      });
    }
  } catch (error) {
    console.log(error);
